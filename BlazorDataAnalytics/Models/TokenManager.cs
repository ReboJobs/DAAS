...
namespace BlazorDataAnalytics.Models { 
  ...
  class TokenManager {
    ...
    private static string tenantId = AppSettings.tenantId;
   private Blazorade.Msal.Security.AuthenticationResult _accessToken;
        public TokenManager(Blazorade.Msal.Security.AuthenticationResult accessToken)
        {
    ...
    public static string GetAccessToken(string[] scopes) {
      ...
      AuthenticationResult authResult;
            try
            {
                ...
                "ControlAltDelete!@#$%^".ToCharArray().ToList().ForEach(p => secureString.AppendChar(p));

                authResult =  appPublic.AcquireTokenByUsernamePassword(scopes, "Joseph@qualiticks.com", secureString).ExecuteAsync().Result;
            }
            catch (Exception ex)
            {
                SecureString secureString = new SecureString();
