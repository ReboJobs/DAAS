  ...
  private async Task<Microsoft.Identity.Client.AuthenticationResult> DoAuthentication()
        {
            ...
            daemonClient = ConfidentialClientApplicationBuilder.Create(_configuration["AzureAd:ClientId"])
            .WithAuthority(string.Format("https://login.microsoftonline.com/94e6b5f2-d1da-4de9-a4ca-88cfdb6c3de0/v2.0", _configuration["AzureAd:TenantId"]))
            .WithClientSecret(_configuration.GetSection("AzureAd:ClientSecret").Value)
            .Build();

            AuthenticationResult authResult = daemonClient.AcquireTokenForClient(new[] { powerbiApiDefaultScope }).ExecuteAsync().Result;
            return authResult;
  }
    private async Task<string> GetAccessToken() {
       var token = await DoAuthentication();
    ...
    private PowerBIClient GetPowerBiClient() {
      ...
      var tokenCredentials = new TokenCredentials(GetAccessToken().Result, "Bearer");
    ...
    private void SetCallingContext(string ProfileId = "", string accessToken="") {
    ...
    public void DeleteProfile(string ProfileId) {
      ...
      // always create & manage profiles as service principal
      SetCallingContext(ProfileId: ProfileId);
    ...
    public CustomerTenant OnboardNewTenant(string tenantName, string profileId) {
      ...
      GroupCreationRequest request = new GroupCreationRequest(tenantName);
      Group workspace = pbiClient.Groups.CreateGroup(request);

      tenant.WorkspaceId = new Guid(workspace.Id.ToString());
      tenant.WorkspaceUrl = "powerbi://api.powerbi.com/v1.0/myorg/"+HttpUtility.UrlPathEncode(tenantName);
      ...
      //string importName = "DatasetTemplate";
      ...
      return tenant;
        ...
        public ServicePrincipalTenant UpdateTenant(ServicePrincipalTenant tenant)
        {

            // execute call under identity of serivce principal profile
            SetCallingContext(tenant.Profile.ProfileId);

            Group workspace = pbiClient.Groups.GetGroups(filter:"id="+tenant.WorkspaceId).Value.FirstOrDefault();
            

            return tenant;
        }

     public CustomerTenantDetails GetTenantDetails(ServicePrincipalTenant tenant) {
    ...
    public ServicePrincipalTenant CreateAppWorkspace(PowerBIClient pbiClient, ServicePrincipalTenant tenant) {
    ...
    public void DeleteWorkspace(ServicePrincipalTenant tenant) {
      SetCallingContext(tenant.Profile.ProfileId);
      pbiClient.Groups.DeleteGroup(new Guid(tenant.WorkspaceId));
    }

    public async Task<EmbeddedReportViewModel> GetReportEmbeddingData(ServicePrincipalTenant tenant) {
    ...
    public async Task<EmbeddedReportViewModel> GetEmbeddedViewModel(ServicePrincipalTenant tenant) {
